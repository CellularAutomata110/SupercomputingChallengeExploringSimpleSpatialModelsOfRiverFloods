from constants import COLOR_CLASSES
from models.grids import SquareGrid, HexGrid
import random_color as rcolor
import numpy as np
import random


class SynchronousCellularAutomataOptionParser:
    def __init__(self, options):
        ...
        self.height = self.process_height(options)
    ...
    def process_height(self, options):
        if self.debug:
            print("Processing initial state... ", end = "")
        if 'height' in options:
            #print("height is indeed in options")
            height = options['height']
            #print("the found height from options[height]")
            #print(height)
            # Check for any new states that don't have color definitions
            random.seed(12312)   # RNG seed for color generation.
            # If numpy version too low, have to iterate without nditer
            np_version = np.__version__.split('.')
            if int(np_version[1]) >= 7:
                for node_state in np.nditer(height, flags = ["refs_ok"]):
                    self.update_colormap(str(node_state))
            else:
                for line in height:
                    for node_state in line:
            ...
            if self.surface_geometry == "square":
                if self.debug:
                    print("init_state: " + str(height))
                    print("shape: " + str(height.shape))
                self.grid_height = SquareGrid(height.shape[0], height.shape[1],
                                       wrap = self.wrap_grid)
            elif self.surface_geometry == "hex":
                self.grid_height = HexGrid(height.shape[0], height.shape[1],
                                    wrap = self.wrap_grid)
            else:
                raise Exception("Surface geometry must be set before " + \
                                "processing initial state.")
            self.grid_height.set_global_state(height)
            if self.debug:
                print("done.")
            #print("ending if statement in options:")
            #print(height)
            return height
        else:
            #print("height has NOT been found in options. Final ELSE statement")
            if self.debug:
                print("done.")
            return None
